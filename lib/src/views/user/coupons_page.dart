...
class _CouponsPageState extends State<CouponsPage> {
  ...
  Future<void> _showCouponDetails({required String couponId, CouponModel? existing}) async {
    ...
    if (coupon == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cupom não encontrado')));
      return;
    }
    StoreModel? store;
    try {
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 12,
                  children: [
                    _DetailChip(icon: Icons.percent, label: 'Desconto', value: coupon.discount.isEmpty ? '-' : coupon.discount),
                    _DetailChip(icon: Icons.check_circle, label: 'Mín. Coletas', value: '${coupon.minCollections}'),
                    _DetailChip(icon: Icons.calendar_today, label: 'Validade', value: validadeStr),
                  ],
                ),
                const SizedBox(height: 20),
                const Text('Como Resgatar', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                if (store != null) ...[
                  Text(store.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                      Text(store.address.isNotEmpty ? store.address : 'Endereço não disponível',
                        style: const TextStyle(fontSize: 13, color: Colors.black54)),
                  const SizedBox(height: 12),
                ] else ...[
                  const Text('Informações da loja não encontradas.', style: TextStyle(color: Colors.redAccent)),
                  const SizedBox(height: 12),
                ],
                const Text('Apresente este cupom ao atendente no ato da compra/coleta para validar o benefício.', style: TextStyle(fontSize: 13)),
                const SizedBox(height: 20),
                Row(
                  children: [
                    Expanded(
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                const Text('Cupons', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                TextButton.icon(
            ...
            Text('Coletas concluídas: ${vm.userCollectionsCount}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
            const SizedBox(height: 12),
            if (vm.isLoading)
            ...
            Expanded(
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Próximos Cupons', style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    FutureBuilder<List<CouponModel>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final claimedIds = vm.claimedCoupons.map((c) => c.couponId).toSet();
                        ...
                        final blockedCoupons = allCoupons.where((c) {
                          ...
                          final bloqueado = vm.userCollectionsCount < c.minCollections;
                          return notExpired && c.quantityAvailable > 0 && !jaResgatado && bloqueado;
                        }).toList();
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                children: const [
                                  ...
                                  SizedBox(height: 8),
                                  Text('Parabéns! Você já liberou todos os cupons disponíveis.'),
                                ],
                              ),
                            ),
                          );
                        ...
                        return Column(
                          children: blockedCoupons.map((c) {
                            final progresso = vm.userCollectionsCount / c.minCollections.clamp(1, double.infinity);
                            final falta = (c.minCollections - vm.userCollectionsCount).clamp(0, double.infinity).toInt();
                    ...
                    const Text('Seus Cupons', style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    if (vm.claimedCoupons.isEmpty)
                      const Text('Nenhum cupom resgatado ainda.', style: TextStyle(fontSize: 12))
                    else
                      Column(
                        children: vm.claimedCoupons.map((uc) {
                          ...
                          return Card(
                            child: ListTile(
                              ...
                              trailing: uc.used
                                  ? const Chip(label: Text('Usado'), backgroundColor: Colors.grey)
                                  : const Chip(label: Text('Disponível'), backgroundColor: Colors.greenAccent),
                              onTap: () => _showCouponDetails(couponId: uc.couponId, existing: coupon),
                            ),
                          );
                        }).toList(),
                      ),
